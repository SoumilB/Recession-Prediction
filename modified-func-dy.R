...
runDY=function(Y_data,indice,lag,type="dynamic"){
  
  Y = Y_data[,c("USREC", "IS")]
  aux=embed(Y,1+lag)
  y= aux[,1]
  
  X=as.matrix(aux[,-c(1:((ncol(Y)*lag)+1))]) # lags of Y (predictors) corresponding to forecast horizon to be removed 
  if(lag==1){ 
    X.out=tail(aux,1)[2:(ncol(X)+1)] #retrieve 4 last observations if one-step forecast 
  }else{
    X.out=aux[,-c(1:(ncol(Y)*(lag-1)))] #delete first (h-1) columns of aux,  
    X.out=tail(X.out,1)[2:(ncol(X)+1)] #last observations: y_T,y_t-1...y_t-h
  }
...
dy.rolling.window=function(Y,nprev,indice=1,lag=1,type = "dynamic"){
  
  save.coef=matrix(NA,nprev,2) #blank matrix for coefficients at each iteration
  ...
  for(i in nprev:1){  #NB: backwards FOR loop: going from 180 down to 1
    ...
    fact=runDY(Y.window,indice,lag) #call the function to fit the AR(p) selected on BIC and generate h-step forecast
  ...
  plot(real,type="l",main = paste("Dynamic Probit:",lag,"- step forecast"))
  lines(c(rep(NA,length(real)-nprev),save.pred),col="red")
  legend("topleft",legend=c("Actual","DP Pred"), col = c("black","red"), lty = 1)
  
  rmse=sqrt(mean((tail(real,nprev)-save.pred)^2)) #compute RMSE
  mae=mean(abs(tail(real,nprev)-save.pred))
  errors=c("rmse"=rmse, "misclassification percentage"=mae) #stack errors in a vector
